import { motion } from 'framer-motion';

const bullets = [
    'galat area mein kitchen nahi khula',
    'loss bacha',
    'profit badha',
];

const Slide61 = () => (
    <section className="slide-section min-h-screen w-full flex items-center justify-center px-8 md:px-16 lg:px-24 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-bl from-premium-black via-premium-black to-subtle-gray" />
        <div className="relative z-10 max-w-6xl w-full space-y-8"><motion.h2 className="font-display text-4xl md:text-5xl lg:text-6xl leading-tight"
                initial={{ opacity: 0, y: 25 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 1, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
            >
                👉 “Ab Samajh Aaya Value Role Kya Hota Hai?”
            </motion.h2>
            <motion.div
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.3 }}
                transition={{ duration: 1, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
                className="space-y-4 text-xl text-text-muted leading-relaxed"
            >
                <p>“Is analysis se:</p>
                <ul className="space-y-2 text-lg">
                    {bullets.map((item) => (
                        <li key={item} className="flex items-start gap-3">
                            <span className="text-premium-gold">–</span>
                            <span>{item}</span>
                        </li>
                    ))}
                </ul>
                <p>“Isliye data ke saath kaam karne wale log:</p>
                <ul className="space-y-2 text-lg">
                    <li className="flex items-start gap-3">
                        <span className="text-premium-gold">•</span>
                        <span>meeting mein suney jaate hain</span>
                    </li>
                    <li className="flex items-start gap-3">
                        <span className="text-premium-gold">•</span>
                        <span>replace nahi hote</span>
                    </li>
                    <li className="flex items-start gap-3">
                        <span className="text-premium-gold">•</span>
                        <span>growth fast hoti hai”</span>
                    </li>
                </ul>
                <p>“Yeh woh log hote hain jo company mein useful hote hain — sirf busy nahi.”</p>
            </motion.div>
        </div>
    </section>
);

export default Slide61;
