...
const Slide32jDataCollectionHighlight = () => (
  <section className="slide-section min-h-screen w-full flex items-center justify-center px-8 md:px-16 lg:px-24 relative overflow-hidden">
    ...
    <div className="relative z-10 max-w-6xl w-full space-y-6">
      ...
      >
        ...
        {steps.map(({ step, tools }) => (
          <div
            key={step}
            className="grid grid-cols-1 md:grid-cols-2 border-t text-base md:text-lg bg-gradient-to-r from-premium-gold/40 via-premium-gold/30 to-transparent border-premium-gold/40"
          >
            <div className="px-4 py-4 font-semibold text-premium-gold">{step}</div>
            <div className="px-4 py-4 leading-relaxed text-white">{tools}</div>
          </div>
        ))}
      </motion.div>
    </div>
...
export default Slide32jDataCollectionHighlight;
